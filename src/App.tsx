  ...
  return (
    <div className="form-page">
      ...
      <form className="registration-form" onSubmit={handleSubmit((values) => mutation.mutate(values))}>
        ...
        <label className="consent-card wide"><input type="checkbox" {...register("socialConsent")} /><span className="custom-check"><Check /></span><span><strong>Redes oficiales de SOPLA</strong><small>Autorizo que mis fotografias puedan ser consideradas para publicacion.</small></span></label>
...
function ReviewPhoto() {
  ...
  const approve = async () => {
    setSaving(true);
    const photo: Photo = { id: crypto.randomUUID(), participantId: participant.id, frameId: selectedFrameId, createdAt: new Date().toISOString(), publicationStatus: "private", delivered: false };
...
function Gallery() {
  ...
  const setPublication = useDemoStore((state) => state.setPublication);
  ...
  const [publicationPhoto, setPublicationPhoto] = useState<string | null>(null);
  ...
  return <Shell back="/kiosk/frames"><div className="gallery-page"><div className="gallery-heading"><div><span className="overline dark-text">GALERIA DE {participant.name.split(" ")[0].toUpperCase()}</span><h1>Momentos para recordar.</h1><p>{photos.length} aprobadas · {participant.photosRemaining} de 4 fotografias disponibles</p></div><button className="button primary" disabled={participant.photosRemaining === 0} onClick={() => navigate("/kiosk/frames")}><Camera /> Tomar otra foto</button></div>{photos.length === 0 ? <div className="empty-gallery"><Image /><h2>Tu galeria esta esperando</h2><p>Elige un marco y captura tu primer momento.</p><button className="button primary" onClick={() => navigate("/kiosk/frames")}>Comenzar <ArrowRight /></button></div> : <div className="gallery-grid">{photos.map((photo) => <article className="gallery-card" key={photo.id}>{images[photo.id] ? <img src={images[photo.id]} alt="Momento SOPLA aprobado" /> : <div className="image-loading"><RefreshCw className="spin" /></div>}<div className="gallery-card-info"><span><strong>{frames.find((frame) => frame.id === photo.frameId)?.name}</strong><small>{new Date(photo.createdAt).toLocaleTimeString("es", { hour: "2-digit", minute: "2-digit" })}</small></span><div><button onClick={() => download(photo)} title="Descargar"><Download /></button><button onClick={() => setPublicationPhoto(photo.id)} title="Autorizar publicacion"><ShieldCheck /></button><button onClick={() => remove(photo.id)} title="Eliminar"><Trash2 /></button></div></div><span className={`publication-tag ${photo.publicationStatus}`}>{photo.publicationStatus === "private" ? "Privada" : "Autorizada para redes"}</span></article>)}</div>}<div className="delivery-panel"><div><span className="whatsapp-icon"><MessageCircle /></span><span><strong>Recibe tu galeria en WhatsApp</strong><small>{participant.whatsapp}</small></span></div>{delivery === "sent" ? <span className="delivery-success"><BadgeCheck /> Galeria entregada correctamente</span> : <button className="button whatsapp" onClick={sendGallery} disabled={delivery === "sending"}>{delivery === "sending" ? "Enviando galeria..." : "Enviar ahora"}<ArrowRight /></button>}</div></div>{publicationPhoto && <div className="modal-backdrop"><div className="consent-modal"><button className="modal-close" onClick={() => setPublicationPhoto(null)}><X /></button><span className="modal-icon"><ShieldCheck /></span><small>AUTORIZACION DE PUBLICACION</small><h2>¿Compartimos este momento?</h2><p>Autorizas que esta fotografia pueda aparecer en las redes oficiales de SOPLA. La publicacion en Meta es simulada en este prototipo.</p><div><button className="button ghost" onClick={() => { setPublication(publicationPhoto, "private"); setPublicationPhoto(null); }}>No, mantener privada</button><button className="button primary" onClick={() => { setPublication(publicationPhoto, "authorized"); setPublicationPhoto(null); }}>Si, autorizo</button></div></div></div>}</Shell>;
}
